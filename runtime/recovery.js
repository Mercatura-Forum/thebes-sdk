...
//   seed  --SHA-256(domain||seed) mod (n-1) +1--> P-256 scalar  mnemonic to seed")
//   scalar --*G--> pubkey (x,y) --> COSE EC2 (RFC 8152 §13.1.1)
...
(function (global) {
    ...
    function modinv(a, m) { return modpow(mod(a, m), m - 2n, m); } // m prime

    // Jacobian coordinates: (X, Y, Z) with x = X/Z^2, y = Y/Z^3.
    function jDouble(pt) {
        var X = pt[0], Y = pt[1], Z = pt[2];
        if (Y === 0n) return [0n, 1n, 0n];
        var S = mod(4n * X * Y * Y, P);
        var Zsq = (Z * Z) % P;
        // M = 3X^2 + aZ^4; a = -3 mod p, use the -3 shortcut form:
        var M = mod(3n * (X - Zsq) * (X + Zsq), P);
        var X2 = mod(M * M - 2n * S, P);
        var Y2 = mod(M * (S - X2) - 8n * Y * Y * Y * Y, P);
        var Z2 = mod(2n * Y * Z, P);
        return [X2, Y2, Z2];
    }
    function jAdd(p1, p2) {
        ...
        var U1 = (p1[0] * Z2Z2) % P, U2 = (p2[0] * Z1Z1) % P;
        var S1 = (p1[1] * Z2Z2 * p2[2]) % P, S2 = (p2[1] * Z1Z1 * p1[2]) % P;
        if (U1 === U2) {
            if (S1 !== S2) return [0n, 1n, 0n];
            return jDouble(p1);
        }
        var H = mod(U2 - U1, P), R = mod(S2 - S1, P);
        var HH = (H * H) % P, HHH = (HH * H) % P;
        var V = (U1 * HH) % P;
        var X3 = mod(R * R - HHH - 2n * V, P);
        var Y3 = mod(R * (V - X3) - S1 * HHH, P);
        var Z3 = mod(H * p1[2] * p2[2], P);
        return [X3, Y3, Z3];
    }
    function jMulG(k) {
    ...
    function jToAffine(pt) {
        if (pt[2] === 0n) throw new Error("point at infinity");
        var zi = modinv(pt[2], P), zi2 = (zi * zi) % P;
    ...
    function bigTo32(v) {
        ...
        for (var i = 31; i >= 0; i--) { out[i] = Number(v & 0xffn); v >>= 8n; }
        return out;
    }
    ...
    // ── BIP39 ───────────────────────────────────────────────────────────────
    // 128-bit entropy -> 12 words (11 bits each; last word carries the 4-bit
    // SHA-256 checksum). Standard BIP39, standard english wordlist.
    async function entropyToPhrase(entropy) {
        if (entropy.length !== 16) throw new Error("entropy must be 16 bytes");
        var check = (await sha256(entropy))[0] >> 4; // first 4 bits
        var bits = [];
        for (var i = 0; i < entropy.length; i++)
            for (var j = 7; j >= 0; j--) bits.push((entropy[i] >> j) & 1);
        for (j = 3; j >= 0; j--) bits.push((check >> j) & 1);
        var words = [];
        for (i = 0; i < 12; i++) {
            var idx = 0;
            for (j = 0; j < 11; j++) idx = (idx << 1) | bits[i * 11 + j];
    ...
    // Validates word membership + the BIP39 checksum. Rejecting a mistyped
    // phrase HERE (128/2048 odds per wrong word are caught by the checksum)
    // beats a confusing "FactorNotFound" from the canister later.
    async function validatePhrase(phrase) {
        var words = normalizePhrase(phrase).split(" ");
        if (words.length !== 12) return false;
        var bits = [];
        for (var i = 0; i < 12; i++) {
            var idx = WORDS.indexOf(words[i]);
            if (idx < 0) return false;
            for (var j = 10; j >= 0; j--) bits.push((idx >> j) & 1);
        }
        var entropy = new Uint8Array(16);
        for (i = 0; i < 128; i++) if (bits[i]) entropy[i >> 3] |= 1 << (7 - (i & 7));
        var check = (await sha256(entropy))[0] >> 4;
        var got = 0;
        for (i = 128; i < 132; i++) got = (got << 1) | bits[i];
        return got === check;
    }
    ...
    async function phraseToSeed(phrase) {
        var key = await crypto.subtle.importKey(
            "raw", utf8.encode(normalizePhrase(phrase)), "PBKDF2", false, ["deriveBits"]);
        var bits = await crypto.subtle.deriveBits(
            { name: "PBKDF2", hash: "SHA-512", salt: utf8.encode("mnemonic"), iterations: 2048 },
            key, 512);
        return new Uint8Array(bits);
    }

    // ── phrase -> deterministic P-256 identity ─────────────────────────────
    async function deriveIdentity(phrase) {
        var seed = await phraseToSeed(phrase);
        var scalarHash = await sha256(concatBytes(utf8.encode("MEMPHIS-RECOVERY-P256-V1"), seed));
        var priv = mod(bytesToBig(scalarHash), N - 1n) + 1n; // in [1, n-1]
        var pub = jToAffine(jMulG(priv));
        var x = bigTo32(pub[0]), y = bigTo32(pub[1]);
        var credFull = await sha256(concatBytes(utf8.encode("MEMPHIS-RECOVERY-CRED-V1"), x, y));
        var credentialId = credFull.slice(0, 16);

        // COSE EC2 P-256 / ES256 (RFC 8152 §13.1.1): {1:2, 3:-7, -1:1, -2:x, -3:y}
        var cose = concatBytes(
        ...
        async function sign(messageBytes) {
            ...
            var ctr = 0;
            for (;;) {
                var kBytes = await sha256(concatBytes(
                    utf8.encode("MEMPHIS-RECOVERY-K-V1"),
                    bigTo32(priv),
                    bigTo32(z),
                    new Uint8Array([ctr])
                ));
                var k = mod(bytesToBig(kBytes), N - 1n) + 1n;
                var rPt = jToAffine(jMulG(k));
                var r = mod(rPt[0], N);
                if (r === 0n) { ctr++; continue; }
                var s = mod(modinv(k, N) * (z + r * priv), N);
                if (s === 0n) { ctr++; continue; }
                if (s > N >> 1n) s = N - s; // low-s
                return derSig(r, s);
            }
        }
    ...
    function derInt(v) {
        ...
        return [0x02, bytes.length].concat(bytes);
    }
    function derSig(r, s) {
        var ri = derInt(r), si = derInt(s);
        var body = ri.concat(si);
        return new Uint8Array([0x30, body.length].concat(body));
    }

    global.MemphisRecovery = {
